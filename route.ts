...
export async function GET(request: NextRequest) {
  try {
    ...
    // Step 5: Fetch individual workflow details to get updatedAt timestamps and enabled status
    const workflowDetailsMap = new Map<string, any>();
    try {
      console.log('Fetching individual workflow details for updatedAt timestamps and enabled status...');
      ...
      for (const [workflowName, workflowData] of workflowMap.entries()) {
        ...
        try {
          ...
          if (detailResponse.ok) {
            ...
            console.log(`Fetched details for workflow: ${workflowName} (enabled: ${detail.enabled})`);
          } else {
    ...
    // Convert workflow map to array for stats, filtering for enabled workflows only
    const workflows: HubSpotWorkflow[] = Array.from(workflowMap.entries())
      .map(([name, data]) => {
        ...
        return {
          ...
          name: data.name,
          type: 'AUTOMATED',
          enabled: details?.enabled ?? true, // Use actual enabled status from API
          insertedAt: 0,
      ...
      .filter(wf => wf.enabled); // Only include enabled workflows

    console.log(`Filtered to ${workflows.length} enabled workflows`);

    // Filter email-workflow pairs to only include pairs from enabled workflows
    const enabledWorkflowNames = new Set(workflows.map(wf => wf.name));
    const filteredEmailPairs = emailWorkflowPairs.filter(pair => 
      enabledWorkflowNames.has(pair.workflowName)
    );
    
    console.log(`Filtered email pairs from ${emailWorkflowPairs.length} to ${filteredEmailPairs.length} (enabled workflows only)`);
    ...
    const dashboardData: DashboardData = { 
      workflows, 
      emails: filteredEmailPairs, // Use filtered pairs instead of all pairs
      enrollmentStats 
    };
    
    console.log(`Returning ${workflows.length} workflows and ${filteredEmailPairs.length} email-workflow pairs`);
