...
const MAX_WORKFLOWS = parseInt(process.env.MAX_WORKFLOWS || '0', 10);
  const PORTAL_ID = process.env.HUBSPOT_PORTAL_ID || ''; // 0 = no cap
const BETWEEN_CALLS_MS = parseInt(process.env.API_DELAY_MS || '250', 10); // throttle to avoid 10-second window limits
...
async function hsGet(url, params, retries = 6) {
  ...
  if (!r.ok) {
    // Gracefully handle missing scopes for stats endpoint
    if (r.status === 403 && url.startsWith('/marketing/v3/emails/statistics/list')) {
      console.warn('Stats endpoint unavailable (missing scopes). Proceeding without metrics.');
      return { results: [] };
    }
    let body;
...
async function get30dStats() {
    ...
    const params = {
      startTimestamp: String(start.getTime()), // ms since epoch as string
...
async function getEmailHtml(eid) {
    try {
      const meta = await getEmail(eid);
      // Try common fields HubSpot returns for email content
      const html = meta.htmlContent || meta.html || meta?.htmlPage?.body || '';
      return typeof html === 'string' ? html : '';
    } catch (e) {
      console.warn('Email HTML fetch error', eid, e);
      return '';
    }
  }
...
export async function fetchInventory() {
    ...
    // Enhance workflow rows with per-workflow action counts (duplicated per row for visibility)
    ...
    for (const eid of emailIds) {
      ...
      const preview_url = PORTAL_ID ? `https://app.hubspot.com/email/${PORTAL_ID}/manage/${eid}` : ''; // reliable in-app preview
   ...
   // Filter to only workflows that actually had email actions (already ensured by how wfRows are built)
    const emailIds = Object.keys(emailToWorkflows).filter(x => /^\d+$/.test(x));
    const stats30d = await get30dStats();
